...
fun FullMapScreenRoute(navController: NavController, mapViewModel: MapsViewModel, authViewModel: AuthViewModel) {
    ...
    // State to handle permission request result
    ...
    // Handle location permission request and location fetching
    LaunchedEffect(Unit) {
    ...
    LaunchedEffect(permissionGranted.value) {
        if (permissionGranted.value) {
            ...
            fusedLocationProviderClient.lastLocation.addOnSuccessListener { location ->
                location?.let {
                    // Convert to Point (Mapbox GeoJSON)
    ...
    // Display a loading indicator if location is not fetched yet
    if (userLocation.value == null) {
        Box(modifier = Modifier.fillMaxSize(), contentAlignment = Alignment.Center) {
            CircularProgressIndicator()
        }
    } else {
        // Once the location is fetched, display the map
        FullMapsScreen(
